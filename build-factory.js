angular.module('buildApp').factory('buildFactory', function($http) {
	...
	var getBuildTypes = function() {
		return $http.get('https://cse-ci.electricimp.com/guestAuth/app/rest/buildTypes?locator=start:0,count:100');
	};
	
	var getBuildStatus = function(id) {
		return $http.get('https://cse-ci.electricimp.com/guestAuth/app/rest/builds?locator=buildType:' + id + ',start:0,count:1&fields=build(id,status,state,buildType(name,id,projectName,paused))');
	};
	
	factory.getRunningBuilds = function() {
		return $http.get('https://cse-ci.electricimp.com/guestAuth/app/rest/builds?locator=running:true');
	};
